...
video_camera = None
global_frame = None
image, pred_img, original, crop_bg, label = None, None, None, None, "--"

# Release camera in body module if already open
body.cap.release()
...
# Route to get the current prediction label as JSON
@app.route("/label")
def label_text():
    return jsonify(label)

# Translation page route — just renders the template (no passing txt)
@app.route("/translate")
def translate():
    # Re-initialize the webcam capture
    body.cap = body.cv2.VideoCapture(0, cv2.CAP_DSHOW)
    return render_template('video_out.html')  # Removed txt=txt.json, not needed
